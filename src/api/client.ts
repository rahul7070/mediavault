...
export async function request<T>(path: string, options: RequestOptions = {}): Promise<T> {
  ...
  if (deduplicate && (!init.method || init.method === 'GET')) {
    ...
    const promise = executeRequestWithRetry<T>(path, init, maxRetries, baseDelayMs).finally(() => {
  ...
  while (true) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (error.isRetryable && attempt < maxRetries) {
          ...
          const delay = retryAfter
            ...
            : Math.min(baseDelayMs * Math.pow(2, attempt - 1) + Math.random() * 200, 4000);
      ...
      if (attempt < maxRetries) {
        attempt++;
        const delay = Math.min(baseDelayMs * Math.pow(2, attempt - 1) + Math.random() * 200, 3000);
        await sleep(delay, init.signal);
        continue;
      }
...
export async function bulkSetStatusChunked(
  ...
  const workers = Array.from({ length: Math.min(chunks.length, CONCURRENCY_LIMIT) }, () =>
    worker(),
  );
...
export function getCollections(signal?: AbortSignal): Promise<{ items: CollectionItem[] }> {
